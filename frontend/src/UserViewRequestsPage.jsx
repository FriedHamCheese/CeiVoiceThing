...
export default function UserViewRequestsPage({APIDomain, userEmail, redirectToHomePage}){
	...
	async function getTickets(){
		...
		try{
			...
			*/
			response = await fetch(APIDomain + '/tickets/userViewRequests/' + userEmail);
		}catch(err){
		...
		try{
			...
			*/
			...
			//Not catching AbortError because react could find its definition
			if(err instanceof TypeError) return setErrorMessage("Couldn't read the request body from the server.");
			if(err instanceof SyntaxError) return setErrorMessage("Couldn't parse the request from the server.");		
			else throw err;
		}
