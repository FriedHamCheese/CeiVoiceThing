    ...
    try {
        ...
        if (userReqCols.length < 2) {
            if (userReqCols.length === 0) {
                console.log('Adding missing columns to UserRequest: tracking_token, createdAt');
                await connection.execute(
        ...
        if (draftTicketCols.length < 3) {
            ...
            if (!existingCols.includes('suggestedAssignee')) {
                console.log('Adding missing column: DraftTicket.suggestedAssignee');
                await connection.execute(
            ...
            if (!existingCols.includes('deadline')) {
                console.log('Adding missing column: DraftTicket.deadline');
                await connection.execute(
                    'ALTER TABLE DraftTicket ADD COLUMN deadline DATETIME AFTER suggestedAssignee'
                );
            }
            if (!existingCols.includes('createdAt')) {
                console.log('Adding missing column: DraftTicket.createdAt');
                await connection.execute(
        ...
        if (newTicketCols.length < 3) {
            ...
            if (!existingCols.includes('deadline')) {
                console.log('Adding missing column: NewTicket.deadline');
                await connection.execute(
                    'ALTER TABLE NewTicket ADD COLUMN deadline DATETIME AFTER status'
                );
            }
            if (!existingCols.includes('createdAt')) {
                console.log('Adding missing column: NewTicket.createdAt');
                await connection.execute(
            ...
            if (!existingCols.includes('updatedAt')) {
                console.log('Adding missing column: NewTicket.updatedAt');
                await connection.execute(
        ...
        if (!draftAssigneeColNames.includes('assigneeEmail') && draftAssigneeColNames.includes('assigneeID')) {
            try {
                console.log('Renaming DraftTicketAssignee.assigneeID to assigneeEmail');
                await connection.execute(
                    'ALTER TABLE DraftTicketAssignee CHANGE COLUMN assigneeID assigneeEmail VARCHAR(64) NOT NULL'
                );
            } catch (error) {
                console.error('Failed to rename DraftTicketAssignee.assigneeID:', error.message);
            }
        }
        ...
        if (draftAssigneeFk.length === 0) {
            try {
                ...
                console.error('Failed to add DraftTicketAssignee assigneeEmail FK:', error.message);
            }
        }
        ...
        if (!newAssigneeColNames.includes('assigneeEmail') && newAssigneeColNames.includes('assigneeID')) {
            try {
                console.log('Renaming NewTicketAssignee.assigneeID to assigneeEmail');
                await connection.execute(
                    'ALTER TABLE NewTicketAssignee CHANGE COLUMN assigneeID assigneeEmail VARCHAR(64) NOT NULL'
                );
            } catch (error) {
                console.error('Failed to rename NewTicketAssignee.assigneeID:', error.message);
            }
        }
        ...
        if (newAssigneeFk.length === 0) {
            try {
                ...
                console.error('Failed to add NewTicketAssignee assigneeEmail FK:', error.message);
            }
        }
